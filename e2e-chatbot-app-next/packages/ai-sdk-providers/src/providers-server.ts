...
export const databricksFetch: typeof fetch = async (input, init) => {
  ...
  // Clean up messages for ChatModel compatibility
  if (requestInit?.body && typeof requestInit.body === 'string') {
    try {
      const body = JSON.parse(requestInit.body);
      if (body.messages && Array.isArray(body.messages)) {
        body.messages = body.messages.map((msg: Record<string, unknown>) => {
          // Remove 'id' - not in ChatModel schema
          const { id, ...rest } = msg;
          // Convert content array to string if needed
          if (Array.isArray(rest.content)) {
            rest.content = (rest.content as Array<Record<string, string>>)
  ...
  // Patch SSE streaming chunks: inject 'id' and 'model' if missing (required by AI SDK)
  // Also optionally log SSE events if LOG_SSE_EVENTS is enabled
  ...
  if (isSSE && response.body) {
    ...
    const patchedStream = new ReadableStream({
      async pull(controller) {
        ...
        // Patch each SSE line: inject 'id' and 'model' if missing
        const patched = text
          .split('\n')
          .map((line) => {
            ...
            try {
              ...
              if (!obj.id) obj.id = 'chatcmpl-rag';
              if (!obj.model) obj.model = 'rag-agent';
              if (LOG_SSE_EVENTS) {
                eventCounter++;
                console.log(`[SSE #${eventCounter}]`, JSON.stringify(obj));
              }
  ...
  async languageModel(id: string): Promise<LanguageModelV3> {
    ...
    const model = await (async () => {
      ...
      switch (endpointDetails.task) {
          ...
          case 'agent/v1/chat':          // ← agregar acá
          case 'llm/v1/chat':
